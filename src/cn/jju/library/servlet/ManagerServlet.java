...
import cn.jju.library.daoImpl.ManagerDaoImpl;
...
public class ManagerServlet extends HttpServlet {
	...
	private ManagerDaoImpl managerDaoImpl = null; // 声明ManagerDAO的对象
    /**
	...
	private void managerQuery(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		String str = null;
		request.setAttribute("managerQuery", managerDaoImpl.query(str));
	...
	private void managerAdd(HttpServletRequest request,
		...
		int ret = managerDaoImpl.insert(managerForm); // 调用添加管理员信息
		if (ret == 1) {
	...
	private void managerModifyQuery(HttpServletRequest request,
		...
		request.setAttribute("managerQueryif", managerDaoImpl
	...
	private void pwdQuery(HttpServletRequest request,
		...
		request.setAttribute("pwdQueryif", managerDaoImpl.query_pwd(managerForm));
	...
	private void managerModify(HttpServletRequest request,
		...
		int ret = managerDaoImpl.update(managerForm); // 调用设置管理员权限的方法
		if (ret == 0) {
	...
	private void managerDel(HttpServletRequest request,
		...
		int ret = managerDaoImpl.delete(managerForm); // 调用删除信息的方法delete()
		if (ret == 0) {
	...
	private void modifypwd(HttpServletRequest request,
		...
		int ret = managerDaoImpl.updatePwd(managerForm);
		if (ret == 0) {
